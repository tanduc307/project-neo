produce_image_fixed <- function(input_data, file_name = "128-fixed") {
    ...
    par(new = TRUE) # tạo trang mới với các parameter vẫn như cũ
    
    par(yaxt = "none") # không in thông tin trục tung
...
produce_image <- function(input_data,
    ...
    ncol(input_data) -> so_nam
    
    library(Cairo)
    
    Cairo::CairoPNG(
        width = 520 + region.px[1]*(so_nam/year_all),
    ...
    if(!is.null(tua_de_sub)) {
    
        mysubtitle <- tua_de_sub    
    ...
    legend(x = leg$rect$left * (so_nam/year_all),
           y = leg$rect$top * (c/d),
           legend = c("Không có dữ liệu", "Không sản xuất"),
           fill = c("grey", "#00ffff"),
           x.intersp = 1,
